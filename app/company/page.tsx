  ...
  return (
    <div className="space-y-8">
      ...
      <div className="bg-gradient-to-r from-primary to-accent rounded-lg p-8 text-primary-foreground">
        <h1 className="text-3xl font-bold mb-2">{t('company.title')}</h1>
      ...
      {!profile ? (
        <div className="bg-card border border-border rounded-lg p-8 text-center space-y-4">
          <h2 className="text-xl font-semibold">{t('company.noProfileYet')}</h2>
          ...
          <div className="bg-card border border-border rounded-lg p-6 space-y-4">
            ...
            <div className="space-y-2">
              <div className={`inline-block px-3 py-1 rounded-full text-sm font-medium ${
                ...
                {profile.is_verified ? t('company.verified') : t('company.verifyPending')}
              </div>
            </div>
          </div>
      ...
      {profile && (
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          <div className="bg-card border border-border rounded-lg p-6 space-y-4">
            ...
            <dl className="space-y-2 text-sm">
              ...
              <div className="flex justify-between">
                ...
                <dd className="font-medium">{t(`taxonomy.business_${profile.business_type}`) || '—'}</dd>
              </div>
              <div className="flex justify-between">
                <dt className="text-muted-foreground">{t('company.tradeLicenseNumber')}</dt>
                <dd className="font-medium">{profile.trade_license_number || '—'}</dd>
              </div>
              <div className="flex justify-between">
          ...
          <div className="bg-card border border-border rounded-lg p-6 space-y-4">
            ...
            <dl className="space-y-2 text-sm">
              <div className="flex justify-between">
                <dt className="text-muted-foreground">{t('company.contactPerson')}</dt>
              ...
              <div className="flex justify-between">
                ...
                <dd className="font-medium text-xs">{profile.contact_email || '—'}</dd>
              </div>
              <div className="flex justify-between">
                <dt className="text-muted-foreground">{t('company.website')}</dt>
                <dd className="font-medium text-xs">{profile.website || '—'}</dd>
              </div>
            </dl>
          </div>
        </div>
